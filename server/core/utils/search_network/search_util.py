...
def search_song_best(title, artist, album):
    ...
    for idx, (s, item) in enumerate(scored, 1):
        ...
        logger.info(f"{idx}. [{item.get('source')}] score={s:.3f} platform_rank={platform_rank} title={item.get('title')} artist={item.get('artist')} album={item.get('album')} cover={item.get('cover')}{trans_mark}")
    if best:
